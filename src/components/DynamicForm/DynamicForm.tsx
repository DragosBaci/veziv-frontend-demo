...
const DynamicForm: React.FC<DynamicFormProps> = ({ formData, id }) => {
    const [method, setMethod] = useState<string>();
    useEffect(() => {
        if (id === 'cardCreate') setMethod('POST');
        else setMethod('PUT');
    }, [id]);
    const [initialData, setInitialData] = useState<any>();
    const { token } = useValidateUser();
    const { fetcher: initialDataFetcher, response: initialDataResponse } = useGetCustomFetch(requestUrls[id as keyof typeof requestUrls]);
    const { fetcher: sendPayload } = usePostCustomFetch<any, any>(requestUrls[id as keyof typeof requestUrls], method);
    ...
    useEffect(() => {
        if (initialDataResponse && id !== 'cardCreate') {
            setInitialData(initialDataResponse);
        }
    }, [initialDataResponse, id]);
